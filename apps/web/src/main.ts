...
app.innerHTML = `<nav class="sidebar" aria-label="Main navigation"><a class="brand" href="#play"><span class="brand-symbol" aria-hidden="true">♞</span>Stock<span>Fly</span></a><a class="nav-link current" href="#play"><span aria-hidden="true">▦</span> Play</a><a class="nav-link" href="#brain"><span aria-hidden="true">◉</span> Brain</a><p class="sidebar-caption">Chess, through<br>a fly’s connectome.</p><a class="asset-credit" href="/pieces/README.txt">Piece credits</a></nav>
  ...
  <details class="decision-details"><summary>Decision details</summary><div id="decision-info" class="muted">Play a move to see the fly’s decision.</div></details></aside></div></main>`;
...
function render(): void {
  ...
  app.querySelector('.badge')!.textContent = modelInfo?.modelLabel ?? (loadFailed ? 'Unavailable' : 'Loading…');
  ...
  if (lastDecision) app.querySelector('#decision-info')!.textContent = `Selected ${lastDecision.selectedMove} · ${lastDecision.settleSteps} steps · trace ${lastDecision.traceId}. Exact from/to/promotion readouts are retained for this decision.`;
}
...
function askFlyToMove(): void {
  ...
  render();
  post({ type: 'position', fen: game.fen(), ...identity, settleSteps: 16 });
}
...
function resetGame(): void {
  ...
  selectedSquare = null; legalTargets = []; lastMove = []; lastDecision = null; pendingPromotion = null;
  app.querySelector<HTMLElement>('.promotion')!.hidden = true;
  app.querySelector('#decision-info')!.textContent = 'Play a move to see the fly’s decision.';
  brain.reset(); statusText = 'New game. Your move.';
  render(); askFlyToMove();
}
...
worker.addEventListener('message', (event: MessageEvent<StockFlyResponse>) => {
  ...
  if (msg.type === 'loaded') {
    ...
    requestAnimationFrame(() => requestAnimationFrame(() => {
      if (!lifecycle.accepts(msg)) return;
      lastDecision = msg; flyThinking = false; lifecycle.finish();
